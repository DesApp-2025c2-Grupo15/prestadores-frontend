...
const Recetas = () => {
  ...
  const handleOpenCreate = () => {
    ...
    setEditorItem({ tipo: "RECETA", estado: "RECIBIDO", afiliadoId: "", medicamento: "", dosis: "" })
    setEditorOpen(true)
  }

  // abrir editor edit (normaliza valores para el formulario)
  const handleOpenEdit = async (id) => {
    const detail = (await getRecetaById(id).catch(() => null)) || data.find((r) => String(r.id) === String(id))
    if (!detail) {
    ...
    if (editorMode === "create") {
      ...
      try {
        ...
        const updated = {
          ...editorItem,
          ...values,
          afiliado: { id: values.afiliadoId ?? editorItem?.afiliado?.id },
          fechaCreacion: values.fechaCreacion ?? editorItem?.fechaCreacion ?? new Date().toISOString(),
        }
  ...
  const handleConfirmChangeState = async ({ newState, motivo = null }) => {
    ...
    try {
      await cambiarEstadoReceta(selectedId, { nuevoEstado: newState, motivo, usuario: "usuario.app" })
        ...
        d.map((it) =>
          String(it.id) === String(selectedId)
            ? {
                ...it,
                estado: newState,
                historialCambios: motivo ? [{ fecha: new Date().toISOString(), usuario: "mock", accion: newState, motivo }, ...(it.historialCambios || [])] : it.historialCambios,
              }
            : it
        )
      )
      if (open && detalle?.id === selectedId)
        setDetalle((prev) => ({
          ...prev,
          estado: newState,
          historialCambios: motivo ? [{ fecha: new Date().toISOString(), usuario: "mock", accion: newState, motivo }, ...(prev.historialCambios || [])] : prev.historialCambios,
        }))
  ...
  const handleConfirmObservation = async ({ estado, motivo }) => {
    if (!selectedId) return
    try {
      await cambiarEstadoReceta(selectedId, { nuevoEstado: estado, motivo, usuario: "usuario.app" })
      ...
      setData((d) =>
        d.map((it) =>
          String(it.id) === String(selectedId)
            ? { ...it, estado, historialCambios: [{ fecha: new Date().toISOString(), usuario: "mock", accion: estado, motivo }, ...(it.historialCambios || [])] }
            : it
        )
      )
      if (open && detalle?.id === selectedId)
        setDetalle((prev) => ({
          ...prev,
          estado,
          historialCambios: [{ fecha: new Date().toISOString(), usuario: "mock", accion: estado, motivo }, ...(prev.historialCambios || [])],
        }))
  ...
  const columns = [
    ...
    { title: "Fecha", dataIndex: "fechaCreacion", key: "fechaCreacion" },
    {
      title: "Detalle",
      key: "detalle",
      render: (_, record) => <Button type="text" icon={<EyeOutlined />} onClick={() => showDetalle(record.id)} />,
    },
    {
      title: "Acciones",
      key: "acciones",
